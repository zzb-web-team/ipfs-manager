        ...
        <div>
          <div class="ipfs_con">
            <div class="ipfs_con_tit">
              ...
              <div class="allnum">{{ topdata.total_dataflow }}</div>
            </div>
            <div class="ipfs_con_tit">
              <div class="alltitle">
                <span>累计传输次数</span>
              </div>
              <div class="allnum">{{ topdata.total_store_times }}次</div>
            </div>
            <div class="ipfs_con_tit">
              <div class="alltitle">
                <span>累计存储容量</span>
              </div>
              <div class="allnum">{{ topdata.total_storeusage }}</div>
            </div>
            <div class="ipfs_con_tit">
              <div class="alltitle">
                <span>累计存储次数</span>
              </div>
              <div class="allnum">{{ topdata.total_output_times }}次</div>
            </div>
          </div>
        </div>
        <div
        ...
        >
          ...
          <div class="ipfs_item_toptwo">
            ...
            <div class="ipfs_item_toptwo_item">
              <span>总存储:</span>
              <span
                >{{ (serdata.totalCap / 1024 / 1024 / 1024).toFixed(2) }}G</span
              >
            </div>
            <div class="ipfs_item_toptwo_item">
              <span>使用存储:</span>
              <span
                >{{
                  (serdata.occupyCap / 1024 / 1024 / 1024).toFixed(2)
                }}G</span
              >
            </div>
            <div class="ipfs_item_toptwo_item">
              ...
              <span
                >{{
                  (serdata.remainingCap / 1024 / 1024 / 1024).toFixed(2)
                }}G</span
              >
            </div>
          </div>
    ...
    <div class="ipfs_item_bottom">
      <div class="ipfs_item_bottom_title">
        <span
          ...
          >节点IP流量</span
        >
        <span :class="[rotate ? 'bluma' : 'eema']" @click="setmap_show('fs')"
          >节点FS存储</span
        >
      </div>
      <div style="padding: 0 37px;">
        ...
        >
          <el-table-column
            ...
            width="180"
          ></el-table-column>
          <el-table-column prop="upstream" label="占用带宽" width="180"
            ><template slot-scope="scope">
              <span v-if="!scope.row.upstream">--</span
              ><span v-else>{{ scope.row.upstream }}</span>
            </template></el-table-column
          >
          <el-table-column prop="dataflow" label="使用流量"></el-table-column>
        ...
        >
          ...
          <el-table-column
            ...
            label="存储内容"
          ></el-table-column>
          <el-table-column prop="content_id" label="内容ID"></el-table-column>
          ...
          <el-table-column prop="user_id" label="渠道ID"></el-table-column>
        </el-table>
      </div>
    </div>
    <fenye
...
export default {
  ...
  created() {
    ...
    if (this.serdata.totalCap * 1 == 0) {
      ...
      this.fs_rema =
        parseInt(this.serdata.remainingCap) / parseInt(this.serdata.totalCap);
    }
  },
  mounted() {
      ...
      ipfs_basic_info(params)
        .then(res => {
          if (res.status == 0) {
            ...
            this.topdata.total_store_times = res.data.total_store_times;
            this.topdata.total_output_times = res.data.total_output_times;
          }
        })
        .catch(error => {});
    },
