...
export default {
	...
	mounted() {
		...
		if (sessionStorage.getItem('northeast_location')) {
			this.setmap_show(
				JSON.parse(sessionStorage.getItem('northeast_location'))
			);
		} else {
			this.gettit();
			this.getipfsdata();
	...
	methods: {
		...
		setmap_show(num) {
			...
			sessionStorage.setItem('northeast_location', JSON.stringify(num));
			this.gettit();
			this.uopset();
			this.$forceUpdate();
		},
		...
		godetail(dat, num) {
			...
			this.$router.push({
				...
				query: {
					...
					address: '/ipfs_northeast_location',
				},
			});
		},
	},
