    ...
    <div>
      ...
      >
        ...
        <el-table-column label="异常类型" width="220">
          <template slot-scope="scope">
            <span v-if="scope.row.badstade == '正常'" style="color:green;">{{
              scope.row.badstade
            }}</span>
            <span v-else style="color:red;">{{ scope.row.badstade }}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="dev_sn"
        ...
        <el-table-column label="CPU温度">
          <template slot-scope="scope">
            <span v-if="scope.row.cputem_color" style="color:red;">{{
              scope.row.cpu_tem
            }}</span>
        ...
        <el-table-column label="内存占用">
          <template slot-scope="scope">
            <span v-if="scope.row.memtem_color" style="color:red;">{{
              scope.row.mem_ratio
            }}</span>
          ...
          if (res.status == 0) {
            if (res.data.dev_detail_info_list) {
              res.data.dev_detail_info_list.forEach((item, index) => {
                ...
                devm.mem_ratio = item.mem_ratio / 100 + "%";
                devm.cap_mem =
                  item.cap_ratio / 100 + "%" + "/" + item.mem_ratio / 100 + "%";
                devm.badstade = "";
                if (item.cpu_tem >= 8000) {
                  devm.badstade = "CPU温度报警" + " / ";
                  devm.cputem_color = "red";
