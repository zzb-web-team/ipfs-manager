<template>
  <div class="content">
    ...
    <div class="org_con">
      ...
      <div class="btn_area">
        <el-button type="primary" @click="nwisible" v-show="menutype.roleC == 1">新建权限分组</el-button>
      </div>
      <!--  -->
      <el-dialog
        :title="title"
      ...
      >
        <el-col :span="19" :offset="4">
          <el-form :model="form" ref="firstruleForm">
            ...
            <p class="tishi">4-20字符，英文字母、汉字、数字组合，可为纯英文、汉字、数字</p>
        ...
        <div slot="footer" class="dialog-footer" style="text-align:center;">
          <el-button @click="resetForm('firstruleForm')">取 消</el-button>
          <el-button type="primary" @click="firstsubmitForm('firstruleForm')">确 定</el-button>
        </div>
      </el-dialog>
      <!--  -->
      <el-table
      ...
      >
        ...
        <el-table-column label="分组描述" prop="description"></el-table-column>
        <el-table-column prop="user" label="用户列表">
          <template slot-scope="scope">
            <span v-if="scope.row.user.length <= 0">--</span>
            <span v-else v-for="(item,index) in scope.row.user" :key="index">{{ item.label }},</span>
          </template>
        </el-table-column>
      ...
      <!--  -->
      <el-dialog title="权限管理" :visible.sync="quanVisible" width="650px" class="organization_dialog">
        <el-tree
          :data="data"
          show-checkbox
          default-expand-all
        ...
        <div slot="footer" class="dialog-footer" style="text-align:center;">
          ...
          <el-button type="primary" @click="queChecked()">确 定</el-button>
        </div>
      </el-dialog>
                    ...
                    if (k.children && k.children.length > 0) {
                      k.children.forEach(i => {
                        ...
                        if (i.insert_status == 1) {
                          ...
                          if (i.label == "账户管理") {
                            obj.label = "新建账户";
                          } else if (i.label == "组织管理") {
                            obj.label = "新建组织";
                          } else if (i.label == "权限管理") {
                            obj.label = "新建权限分组";
                          }
                          obj.label = "添加";
                          i.children.push(obj);
                      ...
                      if (k.insert_status == 1) {
                        ...
                        if (k.label == "账户管理") {
                          obj.label = "新建账户";
                        } else if (k.label == "组织管理") {
                          obj.label = "新建组织";
                        } else if (k.label == "权限管理") {
                          obj.label = "新建权限分组";
                        }
                        obj.label = "添加";
                        k.children.push(obj);
              ...
              children: [
                {
                  ...
                  children: [{ value: item.id, label: item.label }]
                }
              ]
            };
          } else {
            newuser[item.pdepartmentid].children.forEach(xtime => {
