  ...
  <div class="content">
    ...
    <div class="seach">
      ...
      <div v-if="optiondisplay" class="seach_bottom">
        ...
        >
          <el-option value="*" label="全部"></el-option>
          <el-option value="分发加速播放" label="分发加速播放"></el-option>
    ...
    <div>
      <div style="text-align:right;padding: 10px;">
        <el-button type="primary" @click="exportexc()" :disabled="showdisable">导出</el-button>
      </div>
      <el-table
        :data="tableData"
        border
      ...
      > <el-table-column prop="usage" label="用途" width="180"></el-table-column>
        ...
        <el-table-column prop="endTS" label="结束时间">
          <template slot-scope="scope">{{ scope.row.endTS | getymd }}</template>
        </el-table-column>
        <el-table-column prop="timeuse" label="使用时长">
          <template slot-scope="scope">{{ scope.row.timeuse | s_h }}h</template>
        </el-table-column>
